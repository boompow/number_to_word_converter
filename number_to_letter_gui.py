...
root.title("Number to Letter Converter")
...
result = Label(root, text='', bg="#81dbb6", fg="#032730", font="Verdana", anchor=CENTER)
...
def num_to_letter(inputted):
    global result
    number = 0
    inputs = str(inputted).replace(",", "")
    if inputs.isnumeric():
        number = inputs
    ...
    """Section five"""
    # Displaying the result on the GUI if the following conditions are met

    # entry_box.delete(0, END)
    result.destroy()
    if inputs.isnumeric() and len(inputs) <= 66:
        result = Label(root, text=numbers_in_letters, bg="#81dbb6", fg="#032730", font="Verdana", anchor=CENTER)
        result.grid(row=4, column=0, columnspan=2, padx=20, pady=20, ipady=10, ipadx=10)
    elif not inputs.isnumeric():
        result = Label(root, text='Please enter numbers only.\n እባክዎ ቁጥር  ብቻ ያስገቡ።', bg="#81dbb6", fg="#032730",
                       font="Verdana", anchor=CENTER)
        result.grid(row=4, column=0, columnspan=2, padx=20, pady=20, ipady=10, ipadx=10)
    elif len(inputs) > 66:
        result = Label(root, text="I don't have this number in my database.\n It's too high for me!!!"
                                  "\n\n ኡሪሪሪ አረ ዪሄን ቁጥር ማንበብ አልችልም!!!", bg="#81dbb6", fg="#032730",
                       font="Verdana", anchor=CENTER)
        result.grid(row=4, column=0, columnspan=2, padx=20, pady=20, ipady=10, ipadx=10)


entry_box = Entry(root, text="Enter number here", width=30, bg="#81dbb6", font=10)
entry_box.grid(row=2, column=0, columnspan=2, padx=30, pady=0)
convert = Button(root, text="convert/ መልስ", command=lambda: num_to_letter(inputted=entry_box.get()), bg="#2ecc71",
                 fg="#032730", font="Verdana")
convert.grid(row=3, column=0, columnspan=2, pady=10)

label = Label(root, text="Select language/ ቋንቋ ይምረጡ።", bg="white", fg="#032730", font="Verdana", anchor=CENTER)
label.grid(row=0, column=0, columnspan=2, pady=5)

r1 = Radiobutton(root, text="English", variable=var, value="English", bg="white", fg="#032730", font="Verdana",
                 command=lambda: language(lang=var.get()))

r2 = Radiobutton(root, text="አማርኛ", variable=var, value="Amharic", bg="white", fg="#032730", font="Verdana",
                 command=lambda: language(lang=var.get()))
